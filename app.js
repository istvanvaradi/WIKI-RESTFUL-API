...
.post(function(req,res){
    ...
    newArticle.save(function(err){
        if(!err){
            res.send("All good, data has been transferred");
        }else{
            res.send(err);
        }
    });
...
.get(function(req,res){
Article.findOne({title:req.params.articleTitle}, function(err, founArticle){
if(founArticle){
...
.delete(function(req,res){
    Article.deleteOne({title:req.params.articleTitle}, function(err){
        if(!err){
            res.send("Succesfully deleted 1 article");
        }else{
            res.send(err);
        }
    });
